...
const TEAMS = [
  ...
  { name: "T-Bone",        players: ["Juan Soto","José Ramírez","Vinnie Pasquantino","Manny Machado","Pete Crow-Armstrong","Alex Bregman","Seiya Suzuki","Wyatt Langford"] },
  { name: "Volek",         players: ["Shohei Ohtani","Julio Rodríguez","Bobby Witt Jr","Jazz Chisholm Jr.","Bryce Harper","Taylor Ward","Colson Montgomery","Matt Chapman"] },
  { name: "Kramer",        players: ["Matt Olson","Adolis García","Teoscar Hernández","Christian Walker","Nolan Gorman","Oneil Cruz","Christopher Morel","Isaac Paredes"] },
  { name: "NesselJank",    players: ["Kyle Schwarber","Ronald Acuña Jr.","Fernando Tatis Jr.","Ben Rice","Mike Trout","Brandon Lowe","Kyle Stowers","Mickey Moniak"] },
  ...
  { name: "Raybeck",       players: ["Eugenio Suárez","Shea Langeliers","Ketel Marte","Corbin Carroll","Roman Anthony","Jake Burger","Freddie Freeman","Drake Baldwin"] },
...
function teamLast(team, data, selectedMonth, field) {
  if (selectedMonth === 'Total') {
    return team.players
      .map(p => ({ total: playerTotal(data[p]), val: Number(data[p]?.[field])||0 }))
      .sort((a,b) => b.total-a.total)
      .slice(0,5)
      .reduce((s,p) => s+p.val, 0);
  }
  return team.players.reduce((s,p) => s+(Number(data[p]?.[field])||0), 0);
}
function coldColor(rank, total) {
  const t = rank / Math.max(total-1, 1);
  return `rgb(${Math.round(10+t*190)},${Math.round(40+t*175)},${Math.round(100+t*128)})`;
}
...
function GameStatusBadge({ gs }) {
  if (!gs) return null;
  if (gs.status === 'live') return (
    <div style={{ display:'flex', alignItems:'center', gap:'3px', marginTop:'2px' }}>
      <span className="live-dot" style={{ fontSize:'9px', color:'#ff4444' }}>●</span>
      <span style={{ fontSize:'9px', color:'#ff6666', fontWeight:'bold', letterSpacing:'0.5px' }}>IN PROGRESS</span>
    </div>
  );
  if ((gs.status === 'today' || gs.status === 'upcoming') && gs.gameTime) return (
    <div style={{ fontSize:'9px', color:gs.status==='today'?'rgba(204,232,248,0.7)':'rgba(204,232,248,0.45)', marginTop:'2px' }}>
      {gs.status==='today'?'🕐':'📅'} {formatGameTime(gs.gameTime)}
    </div>
  );
  return null;
}

function RecentHRs({ last1, last5, rank }) {
  const fadedColor  = rank < 4 ? 'rgba(255,255,255,0.3)'  : 'rgba(10,30,60,0.25)';
  const activeColor = rank < 4 ? 'rgba(255,255,255,0.85)' : 'rgba(10,30,60,0.7)';
  return (
    <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end', marginTop:'3px' }}>
      ...
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:'9px', color:last5>0?activeColor:fadedColor, fontWeight:last5>0?'bold':'normal' }}>
          {last5>0?`+${last5}`:'—'}
        </div>
        <div style={{ fontSize:'8px', color:fadedColor }}>5 days</div>
      </div>
    </div>
  );
}

function PlayerRecentHRs({ last1, last5 }) {
  return (
    <div style={{ display:'flex', gap:'6px', justifyContent:'flex-end', marginTop:'2px' }}>
      ...
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:'9px', fontFamily:'monospace', color:last5>0?'rgba(204,232,248,0.8)':'rgba(204,232,248,0.3)', fontWeight:last5>0?'bold':'normal' }}>
          {last5>0?`+${last5}`:'—'}
        </div>
        <div style={{ fontSize:'7px', color:'rgba(204,232,248,0.35)' }}>5 days</div>
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  const teamScores = TEAMS.map(team => {
    ...
    const last5 = teamLast(team, data, selectedMonth, 'last5');
    const last1 = teamLast(team, data, selectedMonth, 'last1');
    const playerRows = team.players.map(p => ({
      ...
      last1: Number(data[p]?.last1)||0,
      months: data[p]?.byMonth||{},
      gameStatus: data[p]?.gameStatus||{ status:'unknown' },
    })).sort((a,b) => selectedMonth==='Total' ? b.total-a.total : b.monthHR-a.monthHR);
    return { ...team, score, last5, last1, playerRows };
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#dde3e8', fontFamily:"'Arial Narrow',Arial,sans-serif", color:'#0a1e3c' }}>
      ...
      <div style={{ position:'relative', overflow:'hidden', background:'linear-gradient(180deg,#b8c4cc 0%,#dde6ec 18%,#f0f4f6 40%,#e4eaee 60%,#c8d4da 80%,#b0bec8 100%)' }}>
        <div style={{ height:'8px', background:'linear-gradient(180deg,#0a2a5e,#1a5fa8,#0d3070)' }}/>
        ...
        <div style={{ padding:'14px 20px 0', textAlign:'center', position:'relative', zIndex:2 }}>
          <div style={{ fontSize:'8px', letterSpacing:'7px', color:'#1a4a80', fontWeight:'bold' }}>THE SILVER BULLET</div>
          <div style={{ width:'60%', margin:'4px auto 6px', height:'1px', background:'linear-gradient(90deg,transparent,#4a8ac0,transparent)' }}/>
          ...
          <div style={{ fontSize:'10px', color:'#3a6090', marginBottom:'6px' }}>
        ...
        <div style={{ height:'8px', background:'linear-gradient(180deg,#0d3070,#1a5fa8,#0a2a5e)' }}/>
            ...
            <div key={team.name} style={{ marginBottom:'8px', background:bgColor, border:'1px solid rgba(255,255,255,0.25)', borderRadius:'8px', overflow:'hidden', boxShadow:rank===0?'0 3px 16px rgba(10,30,80,0.4)':'0 1px 4px rgba(0,0,0,0.12)' }}>
              <div style={{ display:'flex', alignItems:'center', padding:'12px 14px', gap:'12px' }}>
                ...
                <div onClick={()=>setExpandedTeam(isExpanded?null:team.name)} style={{ flex:'0 0 120px', cursor:'pointer' }}>
                  ...
                  <div style={{ fontSize:'10px', marginTop:'1px', color:rank<4?'rgba(255,255,255,0.55)':'rgba(10,30,60,0.45)' }}>{team.players.length} players {isExpanded?'▲':'▼'}</div>
                </div>
                ...
                <div onClick={()=>setExpandedTeam(isExpanded?null:team.name)} style={{ textAlign:'right', cursor:'pointer', minWidth:'72px' }}>
                  <div style={{ fontSize:'28px', fontWeight:'900', color:textColor, fontFamily:'monospace', lineHeight:1 }}>{loading?'–':team.score}</div>
                  {!loading && <RecentHRs last1={team.last1} last5={team.last5} rank={rank}/>}
                </div>
              </div>
              ...
              {isExpanded && (
                <div style={{ borderTop:'1px solid rgba(255,255,255,0.15)', padding:'10px 14px', background:'rgba(0,0,0,0.15)' }}>
                  {team.playerRows.map((p, pi) => {
                    ...
                    return (
                      <div key={p.name} style={{ display:'flex', alignItems:'center', padding:'7px 8px', marginBottom:'4px', background:isTop5?'rgba(255,255,255,0.12)':'transparent', borderRadius:'5px', borderLeft:isTop5?'2px solid rgba(255,255,255,0.5)':'2px solid transparent' }}>
                        <div style={{ fontSize:'10px', color:'rgba(255,255,255,0.4)', width:'18px' }}>{pi+1}</div>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:'5px', flexWrap:'wrap' }}>
                            <span style={{ fontSize:'13px', color:isTop5?'#fff':'rgba(255,255,255,0.65)' }}>{p.name}</span>
                            {isTop5 && <span style={{ fontSize:'9px', color:'#cce8f8' }}>★ TOP 5</span>}
                          </div>
                          <GameStatusBadge gs={p.gameStatus}/>
                        </div>
                        <div style={{ textAlign:'right', flexShrink:0 }}>
                          <div style={{ fontSize:'18px', fontWeight:'bold', color:'#cce8f8', fontFamily:'monospace', lineHeight:1 }}>{hr}</div>
                          <PlayerRecentHRs last1={p.last1} last5={p.last5}/>
                        </div>
                      </div>
                    );
                  })}
